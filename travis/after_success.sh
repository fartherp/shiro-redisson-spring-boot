...
if [ $TRAVIS_REPO_SLUG == "fartherp/shiro-redisson-spring-boot" ] && [ "$TRAVIS_BRANCH" == "master" ] && [[ "$commit_message" != *"[maven-release-plugin]"* ]]; then
  ...
  if [ $TRAVIS_JDK_VERSION == "openjdk8" ]; then
    ...
    # Deploy to sonar
    ./mvnw clean org.jacoco:jacoco-maven-plugin:prepare-agent package sonar:sonar -Dsonar.projectKey=fartherp_shiro-redisson-spring-boot -Dsonar.host.url=https://sonarcloud.io -Dsonar.login=0cbcf6c8f0c6921e74a685f37a076659add07c9b
